...
class CRUD(Postgres_DB):
    ...
    def readDB(self, schema, table, column, condition=None):
        if condition==None:
            ...
            sql = "SELECT {column} FROM {schema}.{table} WHERE {condition}".format(column=column, schema=schema, table=table, condition=condition)
        try:
            self.cursor.execute(sql)
    ...
    def updateDB(self, schema, table, column, value, condition):
        sql="UPDATE {schema}.{table} SET ({column})=ROW({value}) WHERE {condition}".format(schema=schema, table=table, column=column, value=value, condition=condition)
        try:
            self.cursor.execute(sql)
            self.db.commit()
        except Exception as e:
